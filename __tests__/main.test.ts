...
describe('action', () => {
  ...
  it('exits with failure if annotation failed to create', async () => {
    ...
    // Verify that all of the core library functions were called correctly
    expect(debugMock).toHaveBeenNthCalledWith(1, 'Sending annotation to Axiom');
    expect(setFailedMock).toHaveBeenCalledWith('Failed to send annotation to Axiom: Unauthorized');
  });
  ...
  it('sets a failed status on error', async () => {
    ...
    // Verify that all of the core library functions were called correctly
    expect(setFailedMock).toHaveBeenNthCalledWith(1, 'Input required and not supplied: axiom_dataset');
    expect(errorMock).not.toHaveBeenCalled();
  });
});
